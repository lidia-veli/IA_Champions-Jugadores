...
class FBRef:
    ...
    ####################################################################################################################
    def get_player_link(self, player_name, year, league, goalkeepers=False):
        # Asegúrate de que los nombres de los jugadores estén normalizados (p. ej., sin espacios adicionales)
        player_name = player_name.strip().lower()
        
        # Obtener los enlaces de los jugadores como se hizo en scrape_complete_scouting_reports
        if goalkeepers:
            player_links_df = self.scrape_stats(year, league, 'goalkeeping')[2]
        else:
            player_links_df = self.scrape_stats(year, league, 'standard')[2]
        
        # Buscar en el DataFrame el enlace del jugador por su nombre
        player_link = player_links_df[player_links_df['Player'].str.lower() == player_name]['Player Link'].values
        
        if len(player_link) > 0:
            # Retorna el primer enlace si hay múltiples jugadores con el mismo nombre
            return player_link[0]
        else:
            # Retorna None o lanza una excepción si el jugador no se encuentra
            return None
